...
// },
// {},
// {lean:true}).sort('author').exec(function(err,books){
//   console.log(books)
// })
//3. Find all the Fiction books, skip the first 2, and display only 3 of them

// Book.find({genres:"Fiction"},{},{lean:true}).skip(2).limit(3).exec(function(err,books){
//   console.log(books)
// })
...
// Person.find({ height:{$gte:180} , salary: {$gte:30000} },{_id:0,height:1,salary:1},{lean:true}).exec((err,books)=>{console.log(books)})
//2. Find all the people who are tall (>180) OR rich (>30000)
// Person.find({$or:[{height:{$gte:180}} , {salary: {$gte:30000}}]  },{_id:0,height:1,salary:1},{lean:true}).exec((err,books)=>{console.log(books)})
//3. Find all the people who have grey hair or eyes, and are skinny (<70)
// Person.find({$and:[{$or:[{eyes:"grey"} , {hair:"grey"}]},{weight:{$lt:70}}]},{_id:0,eyes:1,hair:1,weight:1},{lean:true}).exec((err,books)=>{console.log(books)})
//4. Find people who have at least 1 kid with grey hair
Person.find({kids:{$elemMatch:{hair:"grey"}}},{},{lean:true}).exec(function(err,people){
